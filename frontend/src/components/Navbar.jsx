...
const Navbar = () => {
  ...
  return (
    <div className="flex relative w-full items-center mb-5 justify-between px-4 bg-gray-100 h-16 rounded-2xl">
      ...
      {search ? (
        <Results results={results} navigate={navigate} setInput={setSearch} />
      ) : (
        <></>
      )}
    </div>
...
const Results = ({ results, navigate, setInput = () => {} }) => {
  return (
    <div className=" flex flex-col p-2 gap-4 items-center  absolute rounded-xl overflow-y-auto z-[99] left-[50%] mx-auto -translate-x-1/2 top-[120%] w-11/12 sm:w-[80%]  h-80 bg-[#F6F7F8]">
      {results?.length > 0 ? (
        results?.map((result) => (
          <div
            key={result?._id}
            className="py-8 rounded-xl h-20 overflow-hidden bg-white  w-full justify-between items-center border-b flex hover:bg-gray-100 cursor-pointer"
          >
            <div
              onClick={() => {
              ...
              className="flex w-full h-16 my-2 justify-center  p-2 rounded-2xl overflow-hidden flex-col"
            >
              <div to={`/product/${result?._id}`}>
                <h3 className="text-sm font-semibold">{result?.name}</h3>
                {/* <p className="text-xs  text-gray-500">{result?.description}</p> */}
              </div>
            </div>
            <div className="w-2/12   h-16  overflow-hidden">
              <img
                src={result?.imageUrl}
